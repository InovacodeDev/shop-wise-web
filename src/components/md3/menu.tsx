...
const menuVariants = cva([
    ...
    variants: {
        size: {
            default: "min-w-[8rem] max-w-[280px] py-2", // MD3 menu sizing
            sm: "min-w-[6rem] max-w-[240px] py-1.5",
            lg: "min-w-[12rem] max-w-[320px] py-3"
        }
    },
    defaultVariants: {
        size: "default"
...
>(({ className, inset, ...props }, ref) => (
    <DropdownMenuPrimitive.Label
        ref={ref}
        className={cn(
            "px-3 py-2 text-xs font-medium text-on-surface-variant",
            inset && "pl-8",
            className
        )}
        {...props}
...
const DropdownMenuShortcut = ({
    ...
    return (
        <span
            className={cn("ml-auto text-xs text-on-surface-variant tracking-widest", className)}
            {...props}
        />
    );
};
...
>(({ className, children, ...props }, ref) => (
    <div
        ref={ref}
        className={cn(
            "flex items-center px-3 py-2 text-sm font-medium text-on-surface border-b border-outline-variant",
            className
        )}
        {...props}
    >
...
// Menu Footer component for actions
...
>(({ className, children, ...props }, ref) => (
    <div
        ref={ref}
        className={cn(
            "flex items-center gap-2 px-3 py-2 border-t border-outline-variant mt-1",
            className
        )}
        {...props}
    >
